...
from app.schemas.user import User, UserCreate, UserUpdate, CustomerCreate, WorkerCreate, AdminCreate
...
def create_customer(
    *, db: Session = Depends(deps.get_db), customer_in: CustomerCreate
    ...
    """
    user = crud.user.get_by_id(db, user_id=customer_in.id)
    if user:
        raise HTTPException(
            status_code=400,
            detail="The user with this ID already exists in the system",
        )
    return services.user_service.create_customer(db=db, customer_in=customer_in)
...
def create_worker(
    *,
    ...
    """
    user = crud.user.get_by_id(db, user_id=worker_in.id)
    if user:
        raise HTTPException(
            status_code=400,
            detail="The user with this ID already exists in the system",
        )
    return services.user_service.create_worker(db=db, worker_in=worker_in)
...
def create_admin(
    *,
    ...
    """
    user = crud.user.get_by_id(db, user_id=admin_in.id)
    if user:
        raise HTTPException(
            status_code=400,
            detail="The user with this ID already exists in the system",
        )
    return services.user_service.create_admin(db=db, admin_in=admin_in)
...
def update_user_me(
    *,
    ...
    """
    return services.user_service.update_user(db=db, user_id=current_user.id, user_in=user_in)
