...
class AdminService:
    ...
    @staticmethod
    def get_vehicle_failure_patterns(db: Session) -> List[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def get_negative_feedback_analysis(db: Session, rating_threshold: int = 3) -> List[Dict[str, Any]]:
        """
        ...
        for worker_id in worker_feedback_summary:
            ...
            worker_feedback_summary[worker_id]["total_completed_orders"] = total_orders
            worker_feedback_summary[worker_id]["average_rating"] = avg_rating
            worker_feedback_summary[worker_id]["low_rating_percentage"] = (
                worker_feedback_summary[worker_id]["low_rating_count"] / total_orders * 100
                if total_orders > 0 else 0
            )
        
        return {
            "low_rated_orders": result,
            "worker_performance_summary": list(worker_feedback_summary.values()),
            "total_low_rated_orders": len(result)
        }
